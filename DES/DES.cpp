/*
 * DES algorithm implemented with C++
 * @y-ume
 */
#include <iostream>
#include <iomanip>
#include <fstream>
#include <string>
#include <bitset>
#include <algorithm>
using namespace std;
#define rep(i,a,b) for(int i=(a);i<(b);++i)
#define sz(x) ((int)(x).size())
typedef unsigned long long ull;

// ---------------- tables
int PC_1[] = {57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36,63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4};
int shiftBits[] = {1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
int PC_2[] = {14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32};
int IP[] = {58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7};
int IP_inv[]={40,8,48,16,56,24,64,32,39,7,47,15,55,23,63,31,38,6,46,14,54,22,62,30,37,5,45,13,53,21,61,29,36,4,44,12,52,20,60,28,35,3,43,11,51,19,59,27,34,2,42,10,50,18,58,26,33,1,41,9,49,17,57,25};
int E[]={32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1};
int P[]={16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25};
int S_BOX[8][4][16]={{{14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7},{0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8},{4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0},{15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13}},{{15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10},{3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5},{0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15},{13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9}},{{10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8},{13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1},{13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7},{1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12}},{{7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15},{13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9},{10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4},{3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14}},{{2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9},{14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6},{4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14},{11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3}},{{12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11},{10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8},{9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6},{4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13}},{{4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1},{13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6},{1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2},{6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12}},{{13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7},{1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2},{7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8},{2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11}}};

bool useCBC = false;
string S_key("iostream");
string plaintext("#include<string>") , ciphertext , re_plaintext;
string buff;
ull key , subkeys[16];

void ReadKey() {
  while(true) {
    cout << "input the key, 8 char needed." << endl;
    cout << "default key is \"" << S_key << "\", press enter to continue." << endl;
    getline(cin , buff);
    if(sz(buff) == 0);
    else if(sz(buff) == 8) S_key = buff;
    else continue;
    cout << "current key is :\n" << S_key << "\n----------------" << endl;
    return;
  }
}

void GenerateKeys() {
  key = 0;
  rep(i,0,8) key |= ull(S_key[i]) << ((7 - i) << 3);
  ull key_56 = 0;
  // use PC_1
  rep(i,0,56) key_56 |= ull(key >> (64 - PC_1[i]) & 1) << (55 - i);
  ull left = key_56 >> 28 , right = key_56 & ((1 << 28) - 1);
  // get subkeys
  rep(i,0,16) {
    left = (left << shiftBits[i] | left >> (28 - shiftBits[i])) & ((1 << 28) - 1);
    right = (right << shiftBits[i] | right >> (28 - shiftBits[i])) & ((1 << 28) - 1);
    key_56 = left << 28 | right;
    rep(j,0,48) subkeys[i] |= ull(key_56 >> (56 - PC_2[j]) & 1) << (47 - j);
  }
}

void SelectMode() {
  cout << "select mode :" << endl;
  cout << "          0 : CBC mode" << endl;
  cout << "  otherwise : ECB mode" << endl;
  getline(cin , buff);
  if(buff == "0") useCBC = true;
  cout << "current mode is :\n" << (useCBC ? "CBC" : "ECB") << " mode\n----------------" << endl;
}

void ReadPlain() {
  cout << "input the plain text." << endl;
  cout << "default text is \"" << plaintext << "\", press enter to continue." << endl;
  getline(cin , buff);
  if(sz(buff)) plaintext = buff;
  cout << "current plain text is :\n" << plaintext << endl;
  cout << "----------------" << endl;
}

void PKCS7Padding() {
  cout << "PKCS7Padding start!" << endl;
  plaintext += string(8 - sz(plaintext) % 8 , 8 - sz(plaintext) % 8);
  cout << "current plain text is :\n" << plaintext << endl;
  cout << "----------------" << endl;
}

ull f(ull x,ull key) {
  ull bits = 0 , newbits = 0;
  rep(i,0,48) bits |= ull(x >> (32 - E[i]) & 1) << (47 - i);
  bits ^= key;
  for(int i=0;i<48;i+=6) {
    int row = (bits >> (i + 5) & 1) << 1 | (bits >> i & 1);
    int col = bits >> (i + 1) & 15;
    newbits |= S_BOX[7 - i / 6][row][col] << (i / 6 * 4);
  }
  ull res = 0;
  rep(i,0,32) res |= ull(newbits >> (32 - P[i]) & 1) << (31 - i);
  return res;
}

void Transform(int isEncrypt = 1) { // 1 -> Encrypt , 0 -> Decrypt
  string & from = isEncrypt ? plaintext : ciphertext;
  string & to = (isEncrypt ? ciphertext : re_plaintext) = "";
  ull last = 0;
  for(int c=0;c<sz(from);c+=8) {
    ull plain = 0 , bits = 0;
    rep(i,0,8) plain |= ull((unsigned char)from[c + i]) << ((7 - i) << 3);
    if(isEncrypt) plain ^= last;
    rep(i,0,64) bits |= ull(plain >> (64 - IP[i]) & 1) << (63 - i);
    ull left = bits >> 32 , right = bits & ((1ull << 32) - 1);
    rep(r,0,16) {
      left ^= f(right,subkeys[isEncrypt ? r : 15 - r]);
      std::swap(left , right);
    }
    ull merge = right << 32 | left;
    ull cipher = 0;
    rep(i,0,64) cipher |= ull(merge >> (64 - IP_inv[i]) & 1) << (63 - i);
    if(useCBC) isEncrypt ? (last = cipher) : (cipher ^= last , last = plain);
    rep(i,0,8) to += char(cipher >> ((7 - i) << 3) & 255);
  }
}

int main() {
  ReadKey();
  GenerateKeys();
  SelectMode();
  ReadPlain();
  PKCS7Padding();
  ofstream cipher("cipher.txt") , plain("plain.txt");
  cout << "Encrypt start!" << endl;
  Transform(1);
  cout << "cipher text is :" << endl;
  cout << ciphertext << endl;
  cout << "better to check the cipher.txt." << endl;
  cipher << ciphertext << endl;
  cout << "hex format is :" << endl;
  rep(i,0,sz(ciphertext)) cout << "0x" << hex << setw(2) << setfill('0') << (unsigned int)(unsigned char)(ciphertext[i]) << " ";
  cout << endl;
  cout << "----------------" << endl;
  cout << "Decrypt start!" << endl;
  Transform(0);
  cout << "plain text is :" << endl;
  cout << re_plaintext << endl;
  cout << "better to check the plain.txt" << endl;
  plain << re_plaintext << endl;
  cout << "----------------" << endl;
  cipher.close();
  plain.close();
  return 0;
}
